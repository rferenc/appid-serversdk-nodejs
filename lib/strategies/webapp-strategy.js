...
WebAppStrategy.prototype.authenticate = function (req, options) {
	...
	if (req.query && req.query.error) {
		...
		logger.debug("Error returned in callback ::", req.query.error);
		...
		logger.debug("Handle begin authentication");
...
function retrieveTokens(formData, strategy) {
	...
	request({
		...
		if (err) {
			logger.error("Failed to obtain tokens ::", err, response, body);
			deferred.reject(err);
		} else if (response.statusCode !== 200) {
			try {
				logger.error("Failed to obtain tokens ::", response, body);
				logger.error(tokenEndpoint, clientId, secret, formData);
			...
			Promise.all([TokenUtil.decodeAndValidate(accessTokenString, serviceConfig), TokenUtil.decodeAndValidate(identityTokenString,serviceConfig)])
