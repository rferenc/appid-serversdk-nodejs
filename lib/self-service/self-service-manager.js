...
const MGMT_GET_USERS_PATH = "/cloud_directory/Users";
...
SelfServiceManager.prototype.signUp = function (userData, language = 'en', iamToken) {
	...
	_getIAMToken(iamToken, this.iamApiKey, this.iamTokenUrl).then(function (token) {
		_handleRequest(token, POST, url, userData, {language}, SIGN_UP, deferred);
...
SelfServiceManager.prototype.forgotPassword = function (email, language = 'en', iamToken) {
	...
	_getIAMToken(iamToken, this.iamApiKey, this.iamTokenUrl).then(function (token) {
		_handleRequest(token, POST, url, {email: email}, {language: language}, FORGOT_PASSWORD, deferred);
	}).catch(deferred.reject);
	return deferred.promise;
};
 ...
 */
SelfServiceManager.prototype.deleteUser = function (uuid, userData, iamToken) {
	const deferred = Q.defer();
	let url = this.managementUrl + MGMT_USERS_PATH + uuid;
	_getIAMToken(iamToken, this.iamApiKey, this.iamTokenUrl).then(function (token) {
		_handleRequest(token, DELETE, url, {}, {}, DELETE_USER, deferred);
	}).catch(deferred.reject);
	return deferred.promise;
};

/**
 * @param {string=} query, optional, query string to search for users
 * @param {string=} iamToken, optional, if passed request to the server will use this token.
 * @return {Object} The users SCIM profiles.
 */
SelfServiceManager.prototype.getUsers = function (query, iamToken) {
	 let qs = { query : query } || {};
    const deferred = Q.defer();
    let url = this.managementUrl + MGMT_GET_USERS_PATH;
    _getIAMToken(iamToken, this.iamApiKey, this.iamTokenUrl).then(function (token) {
        _handleRequest(token, GET, url, {} , qs, GET_USERS, deferred);
    }).catch(deferred.reject);
    return deferred.promise;
};
